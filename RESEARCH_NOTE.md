...
The workload generator creates 10–20 sessions, each with a stochastic active/idle pattern (geometric burst lengths, exponential idle gaps, staggered start times). A configurable fraction of sessions reference shared content — a small pool of randomly-generated "documents" — with overlap appearing at random positions in the growing context (not necessarily prefix-aligned). Sharing is detected the way vLLM can reuse it: prompts are hashed in 16-token blocks chained from token 0, so two sessions share a block only when their prompts are identical up to and including it. (An earlier n-gram detector also counted shared text mid-prompt, which vLLM can never reuse because each token's KV depends on its position and on every token before it; it was removed.)
