...
- **Prefix-alignment is a hard constraint, not a measurement choice.** vLLM's prefix cache reuses only a *contiguous prefix from token 0* (blocks are hashed in a chain from the first token, so a mismatch at position k invalidates everything after it). Our overlap detector is alignment-robust and finds shared content wherever it sits, but content it flags mid-context cannot be reused by the engine no matter how we schedule. So a sharing-aware *scheduler* can only exploit sharing that happens to be prefix-aligned; exploiting mid-context sharing needs an engine-side change (block-level dedup, or KV splicing as in CacheBlend) that is explicitly out of scope here. This is the sharpest limitation of the approach and should be stated first in any writeup.

  **The size of this effect is not yet measured.** `--shared-attach-position prefix` vs `random` quantifies it, and that run is in the cell-3d ablation list. Do not quote a number from the mock backend: its block pool is prefix-aligned by construction, so it reproduces this effect trivially and is circular as evidence. The architectural claim above stands on vLLM's hashing scheme; the magnitude has to come from the GPU run.
